...
- Consistency Loss: Bellman 방정식 기반 일관성 제약을 softplus 패널티로 적용
  gap = V(target) - V(reactants) - cost; loss = softplus(-gap)
"""
...
class Trainer:
    ...
    학습 방식:
    1. Consistency Loss: 반응 전후의 가치 일관성 유지 (Bellman gap에 softplus)
       gap = V(target) - V(reactants) - cost; Loss = softplus(-gap)
    ...
    def _pass(self, consistency_data, fitting_data):
        ...
        # gap가 음수(과소평가)일 때만 softplus로 패널티를 줘서 일관성을 유도
        r_gap = target_values - r_values.reshape(-1) - reaction_costs
        consistency_loss = F.softplus(-r_gap).mean()
    ...
    def eval(self):
        ...
        for batch in self.val_consistency_loader:
            ...
            r_values = self.model(reactant_fps, reactant_masks)
            r_gap = target_values - r_values.reshape(-1) - reaction_costs
            consistency_loss.append(F.softplus(-r_gap).mean().item())
